        ...
        + [
            {
                ...
                "confidence": 100,
                "title": "Bounce",
                "source_code": "commercial_feedback",
                "observed_at": now,
            }
        ],
        [],
...
def test_verified_multisource_company_can_be_quality_b_without_news():
    now = datetime.now(timezone.utc)
    signals = [
        {
            "category": "fit",
    ...
    states = [
        {"source_code": code, "status": "success"} for code in ("receita", "email_quality", "rdap")
    ]

    profile = calculate_profile(signals, [], states)
